...
def load_generator_config():
    ...
    if "providers" in generator_config:
        for provider_id, provider_config in generator_config["providers"].items():
            ...
            elif provider_id in ["google", "openai", "openrouter", "ollama", "bedrock", "azure", "dashscope", "vllm"]:
                default_map = {
...
def is_vllm_embedder():
    """
    Check if the current embedder configuration uses vLLM (OpenAIClient with VLLM env vars).

    Returns:
        bool: True if using vLLM embedder, False otherwise
    """
    return EMBEDDER_TYPE == 'vllm'
...
def get_model_config(provider="google", model=None):
    ...
    # Include initialize_kwargs if present (used by vllm and custom providers)
